...
핵심 원칙:
0. [최우선] 뉴스의 메인 테마를 먼저 파악하세요. 메인 테마의 직접 관련주는 반드시 포함해야 합니다.
   - 예) 스테이블코인 입법 뉴스 → 결제/핀테크/거래소 지분 보유주가 반드시 포함되어야 함
   - 메인 테마 관련주를 빠뜨리고 부수 테마만 다루는 것은 분석 실패입니다
   - 검증 규칙(4-1)은 '가짜 연결 차단'이 목적이지, 확실한 메인 테마 종목까지 줄이라는 뜻이 아닙니다
